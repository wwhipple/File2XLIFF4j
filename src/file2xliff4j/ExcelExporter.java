 ...
 *
 */
public class ExcelExporter extends OOoTextExporter implements Converter {
     ...
     * to a document in the original Excel format. Use (besides the XLIFF
     ...
     *        the Excel document. The language is used in constructing a unique
     *        name for the output file. For example, if ja_JP is specified and
     *        the original input file was named myfile.ods, the output file name
     *        is myfile.ja_jp.ods. (Note that the Java Locale's toString
     ...
     * <ol>
     * <li>Because the Excel Exporter is being called, we assume that the type
     *     is EXCEL.
     ...
     *        in ".xls"). This name is used for (at least) two purposes:
     * <ol>
    ...
    public ConversionStatus convert(ConversionMode mode,
        ...
        if (! mode.equals(ConversionMode.FROM_XLIFF)) {
            throw new ConversionException("The Excel Exporter supports"
                    + " only conversions from XLIFF to Excel.");
        }
         ...
         * OpenOffice.org OpenDocument Calc file. If it throws a ConversionException
         * we will just let it propagate to whoever called us.
         */
        status = super.convert(mode, language, phaseName, maxPhase, nativeEncoding, 
                nativeFileType, xliffOriginalFileName + ".ods", baseDir, notifier,
                boundary, outName);
        ...
        /** 
         * Now have OpenOffice.org convert the OpenOffice.org ods file to Excel.
         */
        String docOutFileName = baseDir + File.separator + "$$tsjoof$$." + language.toString() + ".xls";
        OpenOfficeConnection connection = new SocketOpenOfficeConnection();
	try {
		
            DefaultDocumentFormatRegistry formatReg = new DefaultDocumentFormatRegistry();
            
//            // Come up with a unique output file name that ends in "<language>.xls"
//            String docOutFileName = "";
//            // If the name ends with .xls ...
//            if (xliffOriginalFileName.endsWith(".xls")) {
//                // Find where the .xls suffix is
//                int extensionPos = xliffOriginalFileName.lastIndexOf(".xls");
//                // Insert the language code before the .xls suffix.
//                docOutFileName = baseDir + File.separator 
//                        + xliffOriginalFileName.substring(0,extensionPos)
//                        + "." + language.toString() + ".xls";
...
//                }
//            }
//            else {  // Doesn't end with .xls (Hey, whassup?)
//                // Just tack the language and .xls on the end.
...
//            }
            ...
            // The ods input file (created by our superclass) is the original file name
            // with <language>.ods concatinated on the end (something like
            // somefileName.xls.<language>.ods)
            ...
            if (connection.isConnected()) {
                ...
                converter.convert(inputFile, formatReg.getFormatByFileExtension("ods"),
                    outputFile, formatReg.getFormatByFileExtension("xls"));
            }
            else {
                // Restore the target ods file back to its original name:
                ...
                throw(new OpenOfficeConnectException("Unable to connect to OpenOffice.org"
                    + " to generate target Excel Document."));
            }
        }
            ...
            throw(new OpenOfficeConnectException("Unable to communicate with OpenOffice.org" +
                    " to generate target Excel Document: " + tMessage));
        }
        catch(Exception ex) {
            ...
            System.err.println("Unable to generate target Excel document using OpenOffice.org.");
            System.err.println(tMessage);
            throw new ConversionException("Unable to generate target Excel document using OpenOffice.org: "
                    + tMessage);
        }
        ...
        // 1/8/7: Now we need to rename the generated xls file to the name we really
        // want:
        // Come up with a unique output file name that ends in "<language>.xls"
        String newDocOutFileName = "";
        // If the name ends with .xls ...
        if (xliffOriginalFileName.toLowerCase().endsWith(".xls")) {
            // Find where the .xls suffix is
            int extensionPos = xliffOriginalFileName.toLowerCase().lastIndexOf(".xls");
            // Insert the language code before the .xls suffix.
        ...
        // Also rename the target ods file back to its original name:
        File ff = new File(baseDir + File.separator + "$$tsjoof$$."
                + language.toString() + ".ods");
...
//                + "." + language.toString() + ".ods"));
    ...
    public ConversionStatus convert(ConversionMode mode,
        ...
        /** The skiplist is not used by this converter */
