 ...
 */
public class PdfExporterToWord implements Converter {
     ...
     * created from a PDF document) to Word Format.
    ...
    public ConversionStatus convert(ConversionMode mode,
        ...
        // of a language code and .txt, then have OOo convert it to Word.
        String outWordNameOnly = nativeFileName;
        String outWord;

        int lastDot = outWordNameOnly.lastIndexOf(".");
        if (lastDot == -1) {  // Unusual, but no dot!
            outWord = baseDir + File.separator + outWordNameOnly + "."
                + language.toString() + ".doc";
            outWordNameOnly += (language.toString() + ".doc");
        }
        else {
            outWord = baseDir + File.separator 
                + outWordNameOnly.substring(0,lastDot)
                + "." + language.toString() + ".doc";
            outWordNameOnly = outWordNameOnly.substring(0,lastDot)
                + "." + language.toString() + ".doc";
        }
        ...
        // Output an html document initially (OOo converts it to Word later.)
        BufferedWriter htmlWriter = null;  // Where intermediate document is written
        try {
...
//                    }
        ...
        // Now have OOo convert to Word
        String tempDocOutFileName = baseDir + File.separator + "$$tsjoof$$." 
            + language.toString() + ".doc";
        ...
        try {
            ...
            // up with above.
            File outputFile = new File(tempDocOutFileName);
            ...
            if (connection.isConnected()) {
                ...
                throw(new OpenOfficeConnectException("Unable to connect to OpenOffice.org"
                    + " to generate Word translation of PDF document."));
            }
        }
        ...
        // want:
        File ffNewDoc = new File(tempDocOutFileName);
        ffNewDoc.renameTo(new File(outWord));
        
        if (generatedFileName != null) {
            // Tell caller the name of the output file (wo/directories)
            generatedFileName.write(outWordNameOnly);
        }
