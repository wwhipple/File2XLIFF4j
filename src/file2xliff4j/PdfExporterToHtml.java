 ...
 */
public class PdfExporterToHtml implements Converter {
    ...
    public ConversionStatus convert(ConversionMode mode,
        ...
        // of a language code and .txt, then have OOo convert it to HTML.
        ...
        if (lastDot == -1) {  // Unusual, but no dot!
            ...
            outHtmlNameOnly += (language.toString() + ".html");
        }
        else {
            outHtml = baseDir + File.separator 
                + outHtmlNameOnly.substring(0,lastDot)
                + "." + language.toString() + ".html";
            outHtmlNameOnly = outHtmlNameOnly.substring(0,lastDot)
                + "." + language.toString() + ".html";
        }
        ...
        try {
            ...
            htmlWriter = new BufferedWriter(new OutputStreamWriter(
                new FileOutputStream(outHtml), Charset.forName("UTF-8")));
        }
        catch(FileNotFoundException e ) {
...
//                    }
        ...
        if (generatedFileName != null) {
            ...
            generatedFileName.write(outHtmlNameOnly);
        }
